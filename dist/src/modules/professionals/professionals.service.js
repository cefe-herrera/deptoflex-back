...
let ProfessionalsService = class ProfessionalsService {
    ...
    async requestAmbassador(userId) {
        const profile = await this.prisma.professionalProfile.findUnique({ where: { userId } });
        if (!profile)
            throw new common_1.NotFoundException('Professional profile not found');
        if (profile.status === 'ACTIVE') {
            throw new common_1.ConflictException('Already an active ambassador');
        }
        if (profile.ambassadorRequestedAt) {
            throw new common_1.ConflictException('Ambassador request already submitted');
        }
